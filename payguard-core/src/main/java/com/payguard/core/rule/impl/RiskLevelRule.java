...
public class RiskLevelRule implements PaymentRule {
    ...
    public RuleResult evaluate(Transaction transaction) {
        ...
        switch (riskLevel) {
            case HIGH:
                return RuleResult.review("RiskLevelRule", "High risk customer requires manual review");

            case MEDIUM:
                if (transaction.getAmount().compareTo(mediumRiskReviewLimit) > 0) {
                    return RuleResult.review("RiskLevelRule", "Medium risk customer amount exceeds review limit");
                }
                return RuleResult.approved("RiskLevelRule", "Customer risk level is acceptable");
